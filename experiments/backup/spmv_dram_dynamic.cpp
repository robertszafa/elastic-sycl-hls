...
#include "LoadStoreQueueDRAM.hpp"
...
double spmv_kernel(queue &q, std::vector<int> &h_matrix,
  ...
  // LSQ_PIPES.
  using pipes_ld_req_0 = PipeArray<class class_pipes_ld_req_0, req_lsq_dram_t, 32, 2>;
  using pipes_ld_val_0 = PipeArray<class class_pipes_ld_val_0, int, 32, 2>;
  using pipes_st_req_0 = PipeArray<class class_pipe_st_req_0, req_lsq_dram_t, 32, 2>;

  using pipes_st_val_0 = PipeArray<class class_pipe_st_val_0, tagged_val_lsq_dram_t<int>, 32, 2>;

  using pipe_end_lsq_signal_0 = pipe<class class_pipe_end_lsq_signal_0, bool, 2>;

  auto aguEvent = q.single_task<class MainKernel_AGU_0>([=]() [[intel::kernel_args_restrict]] {
    uint tag = 0;
    int ptr = 0; 
    for (int k = 1; k < M; k++) {
      for (int p = 0; p < M; p++) {
        pipes_ld_req_0:: template PipeAt<0>::write({(addr_dram_t) (matrix + row[ptr]), tag});
        pipes_ld_req_0:: template PipeAt<1>::write({(addr_dram_t) (matrix + col[ptr]), tag});
        tag++;
        pipes_st_req_0:: template PipeAt<0>::write({(addr_dram_t) (matrix + row[ptr]), tag });
  ...
  auto lsqEvent_0 =
      LoadStoreQueueDRAM<int, pipes_ld_req_0, pipes_ld_val_0, pipes_st_req_0,
                         pipes_st_val_0, pipe_end_lsq_signal_0, false, 2, 1, 4, 16>(q);

  auto event = q.single_task<MainKernel>([=]() [[intel::kernel_args_restrict]] {
    for (int k = 1; k < M; k++) {
      for (int p = 0; p < M; p++) {
        int res = pipes_ld_val_0::PipeAt<0>::read() + a[p] * pipes_ld_val_0::PipeAt<1>::read();;
        pipes_st_val_0::PipeAt<0>::write({res});
      }
    }
