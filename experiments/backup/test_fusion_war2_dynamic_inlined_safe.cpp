...
constexpr uint MAX_UINT_32 = (1 << 29);
...
/// (Report tools strugle with >1 template params.)
...
std::vector<event> StreamingMemory(queue &q, T *data) {
  ...
  using StoreAckPipes = PipeArray<class _StoreAckPipe, addr_t, 4, NUM_STORES>;
  UnrolledLoop<NUM_STORES>([&](auto iSt) {
    events[iSt] = q.single_task<StorePortKernel<100*MEM_ID + iSt>>([=]() KERNEL_PRAGMAS {
      [[intel::ivdep]]
  ...
  using LoadMuxPredPipes = PipeArray<class _LoadMuxPred, ld_mux_req_t<LOOP_DEPTH, T>, 16, NUM_LOADS>;
  using LoadMuxAckPipes = PipeArray<class _LoadMuxAckPipes, ack_t<LOOP_DEPTH>, 4, NUM_LOADS>;
  UnrolledLoop<NUM_LOADS>([&](auto iLd) {
    q.single_task<LoadPortKernel<100*MEM_ID + iLd>>([=]() KERNEL_PRAGMAS {
      [[intel::ivdep]]
      [[intel::initiation_interval(1)]]
      while (true) {
    ...
    constexpr int LoadPortID = 100*(MEM_ID+1) + iLd;
    q.single_task<LoadValMuxKernel<LoadPortID>>([=]() KERNEL_PRAGMAS {
      ...
      while (true) {
        ...
        if (MuxReq.reuse) {
          ...
          // auto LoadPtr = ext::intel::device_ptr<T>(data + MuxReq.ack.addr);
  ...
  events[1] = q.single_task<StreamingMemoryKernel<MEM_ID>>([=]() KERNEL_PRAGMAS {
    ...
    constexpr uint ST_COMMIT_Q_SIZE = 32;
    ...
    // Shortcut for head of LoadQueue
    ...
    constexpr uint SAFE_LD_Q_SIZE = 2;
    bool SafeLoadValid[NUM_LOADS][SAFE_LD_Q_SIZE];
    addr_t SafeLoadAddr[NUM_LOADS][SAFE_LD_Q_SIZE];
    sched_t SafeLoadSched[NUM_LOADS][LOOP_DEPTH][SAFE_LD_Q_SIZE];
    bool SafeLoadIsMaxIter[NUM_LOADS][LOOP_DEPTH][SAFE_LD_Q_SIZE];
    bool SafeLoadReuse[NUM_LOADS][SAFE_LD_Q_SIZE];
    T SafeLoadReuseVal[NUM_LOADS][SAFE_LD_Q_SIZE];
    bool SafeLoadMuxPipeSucc[NUM_LOADS][SAFE_LD_Q_SIZE];
    bool SafeLoadAddrPipeSucc[NUM_LOADS][SAFE_LD_Q_SIZE];
    ...
    auto checkNoWAW = [&](const auto iSt, const auto iStOther) {
      ...
      const bool otherAddrGreater =
          StoreAckAddr[iStOther] > NextStoreAddrPlusDelay[iSt];
    ...
    while (!EndSignal) {
      ...
      UnrolledLoop<NUM_LOADS>([&](auto iLd) {
        ...
        if (!SafeLoadValid[iLd][0]) {
          ...
          ShiftBundle(SafeLoadAddr[iLd], 0u);
          UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
        ...
        if (SafeLoadValid[iLd][0]) {
          if (!SafeLoadMuxPipeSucc[iLd][0]) {
            ...
            ld_mux_req_t<LOOP_DEPTH, T> MuxReq{SafeLoadReuse[iLd][0], MuxAck,
          ...
          if (SafeLoadMuxPipeSucc[iLd][0] &&
              (SafeLoadReuse[iLd][0] || SafeLoadAddrPipeSucc[iLd][0])) {
        ...
        if (LoadValid[iLd][0] && !SafeLoadValid[iLd][SAFE_LD_Q_SIZE - 1]) {
        // if (LoadValid[iLd][0]) {
          ...
          if (NoRAW) {
            ...
            SafeLoadValid[iLd][SAFE_LD_Q_SIZE - 1] = true;
            SafeLoadAddr[iLd][SAFE_LD_Q_SIZE - 1] = NextLoadAddr[iLd];
            UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
              SafeLoadSched[iLd][iD][SAFE_LD_Q_SIZE-1] = NextLoadSched[iLd][iD];
              SafeLoadIsMaxIter[iLd][iD][SAFE_LD_Q_SIZE-1] = NextLoadIsMaxIter[iLd][iD];
            });
            SafeLoadReuse[iLd][SAFE_LD_Q_SIZE - 1] = Reuse;
            SafeLoadReuseVal[iLd][SAFE_LD_Q_SIZE - 1] = ReuseVal;
        ...
        if (!StoreAllocValid[iSt][ST_ALLOC_Q_SIZE - 1]) {
          ...
          if (succ) {
            ...
            StoreAllocAddrPlusDelay[iSt][ST_ALLOC_Q_SIZE - 1] =
                StoreReq.addr + 32u;
                // StoreReq.addr + RoundUpPow2(ST_COMMIT_Q_SIZE);
                // StoreReq.addr + 2;
            UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
