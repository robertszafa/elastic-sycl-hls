...
std::vector<event> StreamingMemory(queue &q, T *data) {
        ...
        q.single_task<StorePortKernel<StPortId>>([=]() KERNEL_PRAGMAS {
          ...
          [[intel::ivdep]] [[intel::initiation_interval(
            ...
            if (ReqValid && ValValid) {
              // We construct a pointer from only 32 bits (incorrect for >4GB
              // DRAMs). #pragma clang diagnostic push #pragma clang diagnostic
              // ignored "-Wint-to-pointer-cast"
  ...
  events[1] = q.single_task<
    ...
    // We keep track if all store requests sent to the store port have been
    // ACKed.
    ...
    auto checkNoWAW = [&](const auto dst, const auto src) {
      // constexpr DEP_DIR DepDir =
      // DI.STORE_STORE_COMMON_LOOP_DEPTH[iStOther][iSt]; constexpr int
      // EqCheckLoopDepth =
      ...
      bool SrcWillNotTouchAddr =
          SrcSchedEqual && SrcAddrNoDecr &&
          (StoreAckAddr[src] > (NextStoreAddr[dst]));
          // (StoreAckAddr[src] > (NextStoreAddr[dst] + 32));
...
double page_rank_kernel(queue &q, const std::vector<int> &h_row_ptr,
  ...
  using LoadReqPipes_100 =
      PipeArray<class LoadReqPipes_100_, ld_req_t<3, 3>, 16, 1>;
  ...
  // using MemoryStorePipe_0_0 = pipe<class _MemoryStorePipe_0_0, float, 16>;
  ...
  auto event_MainKernel_agu =
      q.single_task<MainKernel_agu>([=]() [[intel::kernel_args_restrict]] {
        ...
        st_req_100_0.sched[2] = SCHED_SENTINEL;
        st_req_100_0.isMaxIter[2] = true;
        StoreReqPipes_100::PipeAt<0, 0>::write(st_req_100_0);
        StoreReqPipes_100::PipeAt<0, 1>::write(st_req_100_0);
      });
        ...
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i += 2) {
            ...
            for (int j = 0; j < rowel; j++) {
              ...
              st_req_100_2.isMaxIter[2] = ((j + 2) == rowel);
  ...
  auto event_MainKernel_3_agu =
      q.single_task<MainKernel_3_agu>([=]() [[intel::kernel_args_restrict]] {
        ...
        for (uint i = 0; i < numNodes; i += 1) {
          ...
          ld_req_100_0.isMaxIter[0] = (i + 1) == numNodes;
          ld_req_100_0.addr = i;
      ...
      q.single_task<MainKernel_1>([=]() [[intel::kernel_args_restrict]] {
        for (uint iter = 0; iter < maxIters; ++iter) {
          for (uint i = 0; i < numNodes; i += 2) {
            int rowel = row_ptr[i + 1] - row_ptr[i];
            for (int j = 0; j < rowel; j++) {
              StoreValPipes_100::PipeAt<1>::write(2.0f);
            }
          }
        }
      });
  ...
  auto event_MainKernel_2 =
      q.single_task<MainKernel_2>([=]() [[intel::kernel_args_restrict]] {
        for (uint iter = 0; iter < maxIters; ++iter) {
          for (uint i = 0; i < numNodes; i += 2) {
            int rowel = row_ptr[i + 1 + 1] - row_ptr[i + 1];
            for (int j = 0; j < rowel; j++) {
              StoreValPipes_100::PipeAt<2>::write(3.0f);
            }
          }
        }
      });
  ...
  auto event_MainKernel_3 =
      q.single_task<MainKernel_3>([=]() [[intel::kernel_args_restrict]] {
        for (uint i = 0; i < numNodes; i += 1) {
          auto p_ld = LoadValPipes_100::PipeAt<0>::read();
          p_new[i] = p_ld;
        }
      });

  auto memEvents_100 =
      StreamingMemory<100, LoadReqPipes_100, LoadValPipes_100,
                      StoreReqPipes_100, StoreValPipes_100, float>(q, p);

  auto event = q.single_task<MainKernel>([=]() [[intel::kernel_args_restrict]] {
    for (uint iter = 0; iter < maxIters; ++iter) {
      for (uint i = 0; i < numNodes; i += 1) {
        StoreValPipes_100::PipeAt<0>::write(1.0f);
      }
    }
  });
