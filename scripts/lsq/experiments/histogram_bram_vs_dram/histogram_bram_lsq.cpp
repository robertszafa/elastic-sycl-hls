...
constexpr int kLatencyPipelinedLSU = 1;
  ...
  #pragma unroll
        ...
        if (i_store_idx > i_store_val) {
          ...
          if (val_store_pipe_succ) {
            ...
            data[store_entries[stq_tail].address] = val_store;
            // PipelinedLSU::store(
            //     device_ptr<value_t>((value_t *)),
            //     val_store);
            store_entries[stq_tail].countdown = int16_t(kLatencyPipelinedLSU / QUEUE_SIZE + 1);
...
double histogram_kernel(queue &q, const std::vector<int> &h_idx,
  ...
  using pipes_ld_req_0 = PipeArray<class pipes_ld_req_0_class, request_lsq_t, 64, 1>;
  ...
  q.single_task<MainKernel_AGU_0>([=]() [[intel::kernel_args_restrict]] {
    ...
    for (int i = 0; i < array_size; ++i) {
      ...
      // LSQ for BRAM, so communicate only idx.
      pipes_ld_req_0::PipeAt<0>::write({int64_t(idx_scalar), tag});
      tag++;
      pipe_st_req_0::write({int64_t(idx_scalar), tag});
    }
