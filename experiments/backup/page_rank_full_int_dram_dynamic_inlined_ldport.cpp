...
using BurstCoalescedLSU = sycl::ext::intel::experimental::lsu<
  ...
  sycl::ext::intel::experimental::prefetch<false>>;
...
std::vector<event> StreamingMemory(queue &q, T *data) {
  ...
  using StorePortReqPipes = PipeArray<class _StorePortReq, st_port_req_t<LOOP_DEPTH, T>, 4, NUM_STORES>;
  ...
  UnrolledLoop<NUM_STORES>([&](auto iSt) {
    ...
    events[iSt] = q.single_task<StorePortKernel<StPortId>>([=]() KERNEL_PRAGMAS {
      ...
      while (true) {
        ...
        Ack = Req.ack;
        // StoreAckPipes::template PipeAt<iSt>::write(Req.ack);
      }
  ...
  using LoadPortPipes = PipeArray<class _LoadPortPipes, ld_port_req_t<LOOP_DEPTH, T>, 0, NUM_LOADS>;
  ...
  UnrolledLoop<NUM_LOADS>([&](auto iLd) {
    ...
    q.single_task<LoadValMuxKernel<LoadPortId>>([=]() KERNEL_PRAGMAS {
      ...
      while (true) {
        ...
        if (!Req.reuse) {
          LoadPortAddrPipes::template PipeAt<iLd>::write(Req.ack.addr);
        }
        ...
        PRINTF("@%u MEM%d LdPort%d a%d (%d, %d, %d) (%d, %d, %d) isReuse=%d "
               "val=%d\n",
               cycle, int(MEM_ID), int(iLd), Req.ack.addr, Req.ack.sched[0],
               Req.ack.sched[1], Req.ack.sched[2], Req.ack.isMaxIter[0],
               Req.ack.isMaxIter[1], Req.ack.isMaxIter[2], Req.reuse, Val);

        // Ensure ld ack is sent after we get the value.
        LoadAckPipes::template PipeAt<iLd>::write(Req.ack);
  ...
  events[1] = q.single_task<StreamingMemoryKernel<MEM_ID>>([=]() KERNEL_PRAGMAS {
    ...
    while (!EndSignal) {
      ...
      UnrolledLoop<NUM_LOADS>([&](auto iLd) {
        ...
        if (succ) {
          PRINTF("@%u MEM%d ld%d ack a%d (%d, %d, %d) (%d, %d, %d)\n", cycle, MEM_ID, int(iLd), 
          ...
          if (NoRAW) {
            ...
            if (succ) {
              if constexpr (NUM_STORES == 1) {
                PRINTF("@%u MEM%d Ld%d a%d (%d, %d, %d) (%d, %d, %d) reuseId=%d    "
                       "NextStore[0]= a%d (%d, %d, %d) (%d, %d, %d)\n", cycle,
                      int(MEM_ID), int(iLd), NextLoadAddr[iLd],
                      NextLoadSched[iLd][0], NextLoadSched[iLd][1],
                      NextLoadSched[iLd][2], NextLoadIsMaxIter[iLd][0],
                      NextLoadIsMaxIter[iLd][1], NextLoadIsMaxIter[iLd][2], reuseId,
                      NextStoreAddr[0], NextStoreSched[0][0], NextStoreSched[0][1], NextStoreSched[0][2], 
                      NextStoreIsMaxIter[0][0], NextStoreIsMaxIter[0][1], NextStoreIsMaxIter[0][2]);
              }
              if constexpr (NUM_STORES == 2) {
                PRINTF("@%u MEM%d Ld%d a%d (%d, %d, %d) (%d, %d, %d) reuseId=%d    "
                       "NextStore[0]= a%d (%d, %d, %d) (%d, %d, %d)    "
                       "NextStore[1]= a%d (%d, %d, %d) (%d, %d, %d)\n", cycle, int(MEM_ID), int(iLd), 
                      NextLoadAddr[iLd], NextLoadSched[iLd][0], NextLoadSched[iLd][1], NextLoadSched[iLd][2], 
                      NextLoadIsMaxIter[iLd][0], NextLoadIsMaxIter[iLd][1], NextLoadIsMaxIter[iLd][2], reuseId,
                      NextStoreAddr[0], NextStoreSched[0][0], NextStoreSched[0][1], NextStoreSched[0][2], 
                      NextStoreIsMaxIter[0][0], NextStoreIsMaxIter[0][1], NextStoreIsMaxIter[0][2],
                      NextStoreAddr[1], NextStoreSched[1][0], NextStoreSched[1][1], NextStoreSched[1][2], 
                      NextStoreIsMaxIter[1][0], NextStoreIsMaxIter[1][1], NextStoreIsMaxIter[1][2]);
              }
      ...
      UnrolledLoop<NUM_STORES>([&](auto iSt) {
        ...
        if (succ) {
          if (stAck.addr != StoreAckAddr[iSt]) {
            PRINTF("@%u MEM%d st%d ack a%d (%d, %d, %d) (%d, %d, %d)\n", cycle, MEM_ID, int(iSt), 
                   stAck.addr, stAck.sched[0], stAck.sched[1], stAck.sched[2],
                   stAck.isMaxIter[0], stAck.isMaxIter[1], stAck.isMaxIter[2]);
          }
        ...
        if (StoreAllocValid[iSt][0] && ((NextStoreValueValid[iSt] && IsSafe) ||
          ...
          if (succ) {
            if constexpr (NUM_LOADS == 1) {
              PRINTF("@%u MEM%d St%d a%d (%d, %d, %d) (%d, %d, %d) val=%d    "
                     "Load[0] ack a%d (%d, %d, %d) (%d, %d, %d), next a%d (%d, %d, %d) (%d, %d, %d) \n",
                     cycle, int(MEM_ID), int(iSt), NextStoreAddr[iSt],
                     NextStoreSched[iSt][0], NextStoreSched[iSt][1], NextStoreSched[iSt][2], 
                     NextStoreIsMaxIter[iSt][0], NextStoreIsMaxIter[iSt][1], NextStoreIsMaxIter[iSt][2], NextStoreValue[iSt], 
                     LoadAckAddr[0], LoadAckSched[0][0], LoadAckSched[0][1], LoadAckSched[0][2],
                     LoadAckIsMaxIter[0][0], LoadAckIsMaxIter[0][1], LoadAckIsMaxIter[0][2],
                     NextLoadAddr[0], NextLoadSched[0][0], NextLoadSched[0][1], NextLoadSched[0][2],
                     NextLoadIsMaxIter[0][0], NextLoadIsMaxIter[0][1], NextLoadIsMaxIter[0][2]
                     );
            }
            if constexpr (NUM_LOADS == 2) {
              constexpr int is = (iSt == 0) ? 1 : 0;
              PRINTF("@%u MEM%d St%d a%d (%d, %d, %d) (%d, %d, %d) val=%d    "
                     "Load[0] ack a%d (%d, %d, %d) (%d, %d, %d), next a%d (%d, %d, %d) (%d, %d, %d)    "
                     "Load[1] ack a%d (%d, %d, %d) (%d, %d, %d), next a%d (%d, %d, %d) (%d, %d, %d)    "
                     "OtherSt ack a%d (%d, %d, %d) (%d, %d, %d), next a%d (%d, %d, %d) (%d, %d, %d) \n",
                     cycle, int(MEM_ID), int(iSt), NextStoreAddr[iSt],
                     NextStoreSched[iSt][0], NextStoreSched[iSt][1], NextStoreSched[iSt][2], 
                     NextStoreIsMaxIter[iSt][0], NextStoreIsMaxIter[iSt][1], NextStoreIsMaxIter[iSt][2], NextStoreValue[iSt], 
                     LoadAckAddr[0], LoadAckSched[0][0], LoadAckSched[0][1], LoadAckSched[0][2],
                     ...
                     NextLoadAddr[0], NextLoadSched[0][0], NextLoadSched[0][1], NextLoadSched[0][2],
                     NextLoadIsMaxIter[0][0], NextLoadIsMaxIter[0][1], NextLoadIsMaxIter[0][2],
                     LoadAckAddr[1], LoadAckSched[1][0], LoadAckSched[1][1], LoadAckSched[1][2],
                     LoadAckIsMaxIter[1][0], LoadAckIsMaxIter[1][1], LoadAckIsMaxIter[1][2],
                     NextLoadAddr[1], NextLoadSched[1][0], NextLoadSched[1][1], NextLoadSched[1][2],
                     NextLoadIsMaxIter[1][0], NextLoadIsMaxIter[1][1], NextLoadIsMaxIter[1][2],

                     StoreAckAddr[is], StoreAckSched[is][0], StoreAckSched[is][1], StoreAckSched[is][2],
                     StoreAckIsMaxIter[is][0], StoreAckIsMaxIter[is][1], StoreAckIsMaxIter[is][2],
                     NextStoreAddr[is], NextStoreSched[is][0], NextStoreSched[is][1], NextStoreSched[is][2],
                     NextStoreIsMaxIter[is][0], NextStoreIsMaxIter[is][1], NextStoreIsMaxIter[is][2]
                     );
            }
