...
double vec_trans_kernel(queue &q, std::vector<int> &h_A, const std::vector<int> &h_b) {
  ...
  auto aguEvent = q.single_task<class MainKernel_AGU_0>([=]() [[intel::kernel_args_restrict]] {
    uint tag = 0;
    for (int i = 0; i < N; i++) {
      pipes_ld_req_0:: template PipeAt<0>::write({(addr_dram_t) (A + i), tag});
      tag++;
      pipes_st_req_0:: template PipeAt<0>::write({(addr_dram_t) (A + b[i]), tag });
    }
  });

  auto lsqEvent_0 =
      LoadStoreQueueDRAM<int, pipes_ld_req_0, pipes_ld_val_0, pipes_st_req_0,
                         pipes_st_val_0, pipe_end_lsq_signal_0, false, 1, 1, 4, 64>(q);

  auto event = q.single_task<MainKernel>([=]() [[intel::kernel_args_restrict]] {
    for (int i = 0; i < N; i++) {
      int d = pipes_ld_val_0::PipeAt<0>::read();
      int res =
