    ...
    auto checkNoRAW = [&](const auto iLd, const auto iSt) {
      ...
      if constexpr (DI.ARE_IN_SAME_LOOP[iLd][iSt]) {
        ...
        if constexpr (stWrapDepth >= 0) {
          ...
          // StAddrNoDecrement = CanCheckPosDepDist;
          if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == BACK) {
...
double page_rank_kernel(queue &q, const std::vector<int> &h_row_ptr,
  ...
  q.single_task<class AGU_l1_2>([=]() [[intel::kernel_args_restrict]] {
    ...
    while (pred_pipe::PipeAt<0>::read()) {
      ...
      st_req_p_new.sched[1] = sched_1;
      st_req_p_new.isMaxIter[0] = is_max_iter_0;
      st_req_p_new.isMaxIter[1] = is_max_iter_1;

      ld_req_p_new.sched[0] = sched_0;
      ld_req_p_new.sched[1] = sched_1;
      ld_req_p_new.isMaxIter[0] = is_max_iter_0;
