...
double spmv_if_kernel(queue &q, std::vector<int> &h_w,
  ...
  using pipe_pred = pipe<class class_pipe_pred, bool, 32>;
  using pipe_j = pipe<class class_pipe_j, int, 32>;
  
  auto PE_event = q.single_task<class PE>([=]() [[intel::kernel_args_restrict]] {
    [[intel::ivdep]]
    while (pipe_pred::read()) {
      auto j = pipe_j::read();
      [[intel::ivdep]]
      for (int i = 0; i < M; i++) {
