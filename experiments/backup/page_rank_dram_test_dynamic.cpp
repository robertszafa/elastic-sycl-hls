...
template <int LOOP_DEPTH> struct st_req_t {
  addr_t addr;
  sched_t sched[LOOP_DEPTH];
  bool isMaxIter[LOOP_DEPTH];
};

template <int NUM_STORES, int LOOP_DEPTH> struct ld_req_t {
  addr_t addr;
  sched_t sched[LOOP_DEPTH];
  bool isMaxIter[LOOP_DEPTH];
...
template <int LOOP_DEPTH> using ack_t = st_req_t<LOOP_DEPTH>;

template <int LOOP_DEPTH, typename T> struct ld_port_req_t {
  ack_t<LOOP_DEPTH> ack;
  T val;
  bool reuse;
};

template <int LOOP_DEPTH, typename T> struct st_port_req_t {
  T val;
  ack_t<LOOP_DEPTH> ack;
};

  #define KERNEL_PRAGMAS                                                       \
...
[[clang::optnone]] std::vector<event> StreamingMemory(queue &q) {
  ...
  using StorePortValPipes =
      PipeArray<class _StorePortValPipes, T, 1, NUM_STORES>;
  using StoreAckPipes =
      PipeArray<class _StoreAckPipe, ack_t<LOOP_DEPTH>, 2, NUM_STORES>;
  UnrolledLoop<NUM_STORES>([&](auto iSt) {
    constexpr int StPortId = 100 * (MEM_ID + 1) + iSt;
    events[iSt] =
        q.single_task<StorePortKernel<StPortId>>([=]() KERNEL_PRAGMAS {
          ack_t<LOOP_DEPTH> NextAck{INIT_ACK_ADDR};
          InitBundle(NextAck.sched, 0u);
          InitBundle(NextAck.isMaxIter, true);

          [[intel::ivdep]] [[intel::initiation_interval(1)]] while (true) {
            StoreAckPipes::template PipeAt<iSt>::write(NextAck);

            const auto Req = StoreReqPipes::template PipeAt<iSt, 0>::read();
            if (Req.addr == STORE_ADDR_SENTINEL)
              break;
  ...
  // Load ports.
  using LoadPortReqPipes =
      PipeArray<class _LoadPortPipes, ld_port_req_t<LOOP_DEPTH, T>, 1,
                NUM_LOADS>;
  using LoadAckPipes =
      PipeArray<class _LoadAckPipes, ack_t<LOOP_DEPTH>, 2, NUM_LOADS>;
  ...
  #pragma clang diagnostic pop
      ...
      // PRINTF("MEM%d ld%d reused %d/%d\n", MEM_ID, int(iLd), NumReused,
      // NumTotal);
    });
  });

  events[1] = q.single_task<
      StreamingMemoryKernel<MEM_ID>>([=]() KERNEL_PRAGMAS {
    ...
    // the actual ACK received from the memory system (no access to thaty in
    // HLS).
    ...
    UnrolledLoop<NUM_LOADS>([&](auto iLd) {
      ...
      UnrolledLoop<NUM_STORES>(
          [&](auto iSt) { InitBundle(LoadPosDepDist[iLd][iSt], false); });
      UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
    ...
    auto checkNoWAR = [&](const auto iLd, const auto iSt) {
      ...
      if constexpr (CmnLoopDepth >= 0) {
        constexpr int LoadWrapFromCmn =
            walkUpLoadToFirstWrap(iLd, CmnLoopDepth);
    ...
    auto checkNoWAW = [&](const auto iSt, const auto iStOther) {
      constexpr int CmnLoopDepth =
          DI.STORE_STORE_COMMON_LOOP_DEPTH[iSt][iStOther];
      ...
      if constexpr (CmnLoopDepth >= 0) {
        ...
        if constexpr (EqCheckLoopDepth >= 0) {
          ...
          if constexpr (DepDir == BACK) {
            OtherSchedEqual |= ((StoreAckSched[iStOther][EqCheckLoopDepth] +
                                 1) == NextStoreSched[iSt][EqCheckLoopDepth]);
          }
        }
    ...
    [[intel::ivdep]] [[intel::initiation_interval(1)]] while (!EndSignal) {
      cycle++;
  ...
  #pragma unroll
          ...
          if (NoRAW) {
            ...
            if (succ) {
              LastSentLoadSched[iLd] =
                  NextLoadSched[iLd][DI.LOAD_LOOP_DEPTH[iLd]];
      ...
      UnrolledLoop<NUM_STORES>([&](auto iSt) {
        ...
        if (!StoreValid[iSt][ST_ALLOC_Q_SIZE - 1]) {
          ...
          UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
            ...
            StoreIsMaxIter[iSt][iD][ST_ALLOC_Q_SIZE - 1] =
        ...
        if (StoreValid[iSt][0] && NextStoreValueValid[iSt] && IsSafe) {
          bool succ = false;
          StorePortValPipes::template PipeAt<iSt>::write(NextStoreValue[iSt],
                                                         succ);
...
double page_rank_kernel(queue &q, const std::vector<int> &h_row_ptr,
  ...
  using LoadReqPipes_100 =
      PipeArray<class LoadReqPipes_100_, ld_req_t<2, 3>, 16, 1>;
  using LoadValPipes_100 = PipeArray<class LoadValPipes_100_, float, 16, 1>;
  using StoreReqPipes_100 =
      PipeArray<class StoreReqPipes_100_, st_req_t<3>, 16, 2, 2>;
  using StoreValPipes_100 = PipeArray<class StoreValPipes_100_, float, 16, 2>;
      ...
      q.single_task<MainKernel_agu>([=]() [[intel::kernel_args_restrict]] {
        ...
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i += 2) {
            ...
            ld_req_101_0.isMaxIter[1] = (i+2) == numNodes;

            ld_req_101_1.sched[1]++;
            ld_req_101_1.isMaxIter[1] = (i+2) == numNodes;
            st_req_100_0.sched[1]++;
            st_req_100_0.isMaxIter[1] = (i+2) == numNodes;
  
            ld_req_101_0.addr = reinterpret_cast<uintptr_t>(p + i);
            ld_req_101_1.addr = reinterpret_cast<uintptr_t>(p + i+1);
            ...
            for (int j = 0; j < rowel; j++) {
              st_req_100_0.addr = reinterpret_cast<uintptr_t>(p_new + col_idx[local_curcol]);
      ...
      q.single_task<MainKernel_1_agu>([=]() [[intel::kernel_args_restrict]] {
        ...
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i += 2) {
            ...
            st_req_100_1.isMaxIter[1] = (i+2) == numNodes;

            int rowel = row_ptr[i + 1] - row_ptr[i];
            curcol += rowel;
            rowel = row_ptr[i + 1 + 1] - row_ptr[i + 1];
            int local_curcol2 = curcol;
            curcol += rowel;

            for (int j = 0; j < rowel; j++) {
              st_req_100_1.addr = reinterpret_cast<uintptr_t>(p_new + col_idx[local_curcol2]);
      ...
      q.single_task<MainKernel_2_agu>([=]() [[intel::kernel_args_restrict]] {
        ...
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i++) {
            ...
            ld_req_100_0.addr = reinterpret_cast<uintptr_t>(p_new + i);
            st_req_101_0.addr = reinterpret_cast<uintptr_t>(p + i);
      ...
      q.single_task<MainKernel_1>([=]() [[intel::kernel_args_restrict]] {
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i += 2) {
            ...
            curcol += rowel;
            for (int j = 0; j < rowel; j++) {
              auto p_new_load = 2.0f;
              auto p_new_store = p_new_load + (val[local_curcol] * p_val);
              StoreValPipes_100::PipeAt<0>::write(p_new_store);
              local_curcol++;
            }
          }
        }
      ...
      q.single_task<MainKernel_2>([=]() [[intel::kernel_args_restrict]] {
        for (uint iter = 0; iter < maxIters; ++iter) {
          ...
          for (uint i = 0; i < numNodes; i += 2) {
            ...
            for (int j = 0; j < rowel; j++) {
              auto p_new_load = 2.0f;
              auto p_new_store = p_new_load + (val[local_curcol2] * p_val);
  ...
  auto memEvents_100 = StreamingMemory<100, LoadReqPipes_100, LoadValPipes_100,
                                       StoreReqPipes_100, StoreValPipes_100>(q);
  auto memEvents_101 = StreamingMemory<101, LoadReqPipes_101, LoadValPipes_101,
                                       StoreReqPipes_101, StoreValPipes_101>(q);

  auto event = q.single_task<MainKernel>([=]() [[intel::kernel_args_restrict]] {
    for (uint iter = 0; iter < maxIters; ++iter) {
      
      for (uint i = 0; i < numNodes; i++) {
        auto p_val = d * LoadValPipes_100::PipeAt<0>::read() + (1.0f - d) / numNodes;
...
void page_rank_cpu(const std::vector<int> &row_ptr,
                   ...
                   const int maxIters) {
  std::vector<float> p_new(p.size(), 1.0f);
  for (uint iter = 0; iter < maxIters; ++iter) {
    int curcol = 0;
    for (uint i = 0; i < numNodes; i += 2) {
      auto p_val = p[i];
      ...
      for (int j = 0; j < rowel; j++) {
        auto p_new_load = 2.0f;
        auto p_new_store = p_new_load + (val[local_curcol2] * p_val);
        p_new[col_idx[local_curcol2]] = p_new_store;
        local_curcol2++;
      }
    }
    for (uint i = 0; i < numNodes; i++) {
...
int main(int argc, char *argv[]) {
  ...
  std::vector<float> val(sizeArr, 0.0f), p(sizeArr, 1.0f);
  int fromnode, tonode;
  int cur_row = 0;
  int i = 0;
  int elrow = 0;
  ...
  try {
    ...
    std::vector<float> p_cpu(sizeArr, 1.0f);
