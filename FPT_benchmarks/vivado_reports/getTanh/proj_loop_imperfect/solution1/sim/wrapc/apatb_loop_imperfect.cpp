...
int idx[1000])
{
	...
	else
	{
		...
		// "idx"
		char* tvin_idx = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_idx);
		...
		// [[transaction]]
		sprintf(tvin_idx, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_idx, tvin_idx);

		sc_bv<32>* idx_tvin_wrapc_buffer = new sc_bv<32>[1000];
			...
			// bitslice(31, 0)
			{
				...
				// celement: idx(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 999; i_0 += 1)
					{
						...
						// ori_name              : idx[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : idx[0]
						// regulate_c_name       : idx
						// input_type_conversion : idx[i_0]
						if (&(idx[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> idx_tmp_mem;
							idx_tmp_mem = idx[i_0];
							idx_tvin_wrapc_buffer[hls_map_index].range(31, 0) = idx_tmp_mem.range(31, 0);
                                 	       hls_map_index++;
						}
					}
				}
		...
		for (int i = 0; i < 1000; i++)
		{
			sprintf(tvin_idx, "%s\n", (idx_tvin_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_idx, tvin_idx);
		}

		tcl_file.set_num(1000, &tcl_file.idx_depth);
		sprintf(tvin_idx, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_idx, tvin_idx);

		// release memory allocation
		delete [] idx_tvin_wrapc_buffer;

// [call_c_dut] ---------->

		CodeState = CALL_C_DUT;
		loop_imperfect(A, idx);
