...
template <typename T>
inline bool almost_equal(const dt::Point2D<T> &v1, const dt::Point2D<T> &v2) {
  return almost_equal(v1.x, v2.x) && almost_equal(v1.y, v2.y);
}

inline bool almost_equal(const dt::Edge &e1, const dt::Edge &e2) {
  return (almost_equal(e1.fromPoint, e2.fromPoint) &&
          almost_equal(e1.toPoint, e2.toPoint)) ||
         (almost_equal(e1.fromPoint, e2.toPoint) &&
          almost_equal(e1.toPoint, e2.fromPoint));
}

template <typename T>
inline bool containsPoint(const dt::Point2D<T> *points,
                          const dt::Triangle &triangle,
                          const dt::Point2D<T> &point) {
  return almost_equal(points[triangle.aIdx], point) ||
         almost_equal(points[triangle.bIdx], point) ||
         almost_equal(points[triangle.cIdx], point);
}

template <typename T> T norm2(const dt::Point2D<T> &p) {
  return p.x * p.x + p.y * p.y;
}

template <typename T>
inline T dist2(const dt::Point2D<T> &p1, const dt::Point2D<T> &p2) {
...
template <typename T>
inline bool circumCircleContains(const dt::Triangle &triangle,
                                 const dt::Point2D<T> &point,
...
double dt_k(queue &q, const std::vector<dt::Point2D<float>> &h_points,
  ...
  dt::Point2D<float> *points =
      fpga_tools::toDevice(h_points.data(), h_points.size(), q);
  dt::Triangle *triangles =
      fpga_tools::toDevice(h_triangles.data(), h_triangles.size(), q);
      ...
      for (int iP = 0; iP < numPoints; ++iP) {
        ...
        for (int iT = 0; iT < numTriangles; ++iT) {
          if (circumCircleContains(triangles[iT], points[iP], points)) {
            ...
            polygon[numEdgesInPolygon++] = {triangles[iT].aIdx,
                                            triangles[iT].bIdx, false};
            polygon[numEdgesInPolygon++] = {triangles[iT].bIdx,
                                            triangles[iT].cIdx, false};
            polygon[numEdgesInPolygon++] = {triangles[iT].cIdx,
                                            triangles[iT].aIdx, false};
          }
        }
        ...
        for (int iE = 0; iE < numEdgesInPolygon; ++iE) {
          if (!polygon[iE].isBad) {
            triangles[numGoodTriangles++] = {polygon[iE].fromPoint,
                                             polygon[iE].toPoint, iP, false};
          }
        }
...
template <typename T>
void init_data(std::vector<dt::Point2D<T>> &points,
               std::vector<dt::Triangle> &triangles, const int numPoints,
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    init_data(points, triangles, N, PERCENTAGE);

    auto kernel_time = dt_k(q, points, triangles, N);
