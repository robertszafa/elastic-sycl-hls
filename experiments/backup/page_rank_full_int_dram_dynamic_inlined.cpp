...
std::vector<event> StreamingMemory(queue &q, T *data) {
  ...
  events[1] = q.single_task<StreamingMemoryKernel<MEM_ID>>([=]() KERNEL_PRAGMAS {
    ...
    sched_t StoreAllocSchedMinusOne[NUM_STORES][LOOP_DEPTH][ST_ALLOC_Q_SIZE];
    sched_t StoreAllocSchedPlusOne[NUM_STORES][LOOP_DEPTH][ST_ALLOC_Q_SIZE];
    ...
    sched_t NextStoreSchedMinusOne[NUM_STORES][LOOP_DEPTH];
    sched_t NextStoreSchedPlusOne[NUM_STORES][LOOP_DEPTH];
    ...
    UnrolledLoop<NUM_STORES>([&] (auto iSt) {
      ...
      UnrolledLoop<LOOP_DEPTH>([&] (auto iD) {
        ...
        InitBundle(StoreAllocSchedPlusOne[iSt][iD], 0u);
        InitBundle(StoreAllocSchedMinusOne[iSt][iD], 0u);
      ...
      InitBundle(NextStoreSchedMinusOne[iSt], 0u);
      InitBundle(NextStoreSchedPlusOne[iSt], 0u);
    ...
    auto checkNoRAW = [&](const auto iLd, const auto iSt) {
      ...
      if constexpr (cmnLoopDepth >= 0) {
        ...
        if constexpr (eqCheckLoopDepth >= 0) {
          ...
          if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == BACK) {
            ...
            storeSchedEqual |= (NextStoreSchedPlusOne[iSt][eqCheckLoopDepth] ==
                                NextLoadSched[iLd][eqCheckLoopDepth]);
          }
        }
        ...
        if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == BACK) {
          ...
          storeSchedGreater = (NextStoreSchedPlusOne[iSt][cmnLoopDepth] >
      ...
      if constexpr (DI.ARE_IN_SAME_LOOP[iLd][iSt]) {
        ...
        if constexpr (stWrapDepth >= 0) {
          ...
          CanCheckPosDepDist = (NextStoreSchedPlusOne[iSt][stWrapDepth] >
                                NextLoadSched[iLd][stWrapDepth]);

          if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == BACK) {
            // StAddrNoDecrement = ((NextStoreSched[iSt][stWrapDepth] + 1) >=
            StAddrNoDecrement = (NextStoreSchedPlusOne[iSt][stWrapDepth] >=
                                 NextLoadSched[iLd][stWrapDepth]);
          } else {
      ...
      if constexpr (cmnLoopDepth >= 0) {
        ...
        if constexpr (eqCheckLoopDepth >= 0) {
          ...
          if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == FORWARD) {
            ...
            //                    NextStoreSched[iSt][eqCheckLoopDepth]);
            loadSchedEqaul |= (LoadAckSched[iLd][eqCheckLoopDepth] ==
                               NextStoreSchedMinusOne[iSt][eqCheckLoopDepth]);
          }
        }
        ...
        if constexpr (DI.LOAD_TO_STORE_DEP_DIR[iLd][iSt] == FORWARD) {
          ...
          ackLoadSchedGreater = (LoadAckSched[iLd][cmnLoopDepth] >
                                 NextStoreSchedMinusOne[iSt][cmnLoopDepth]);
          nextLoadSchedGreater = (NextLoadSched[iLd][cmnLoopDepth] >
                                  NextStoreSchedMinusOne[iSt][cmnLoopDepth]);
        } else {
          ackLoadSchedGreater = (LoadAckSched[iLd][cmnLoopDepth] >
      ...
      UnrolledLoop<NUM_STORES>([&](auto iSt) {
        ...
        if (!StoreAllocValid[iSt][0]) {
          if (StoreAllocValid[iSt][1]) {
            ...
            UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
              ...
              NextStoreSchedPlusOne[iSt][iD] = StoreAllocSchedPlusOne[iSt][iD][1];
              NextStoreSchedMinusOne[iSt][iD] = StoreAllocSchedMinusOne[iSt][iD][1];
              // NextStoreSchedMinusOne[iSt][iD] = StoreAllocSched[iSt][iD][1] - 1;
              // NextStoreSchedPlusOne[iSt][iD] = StoreAllocSched[iSt][iD][1] + 1;
          ...
          UnrolledLoop<LOOP_DEPTH>([&] (auto iD) {
            ...
            ShiftBundle(StoreAllocSchedPlusOne[iSt][iD], 0u);
            ShiftBundle(StoreAllocSchedMinusOne[iSt][iD], 0u);
        ...
        if (!StoreAllocValid[iSt][ST_ALLOC_Q_SIZE - 1]) {
          ...
          UnrolledLoop<LOOP_DEPTH>([&](auto iD) {
            ...
            StoreAllocSchedMinusOne[iSt][iD][ST_ALLOC_Q_SIZE - 1] = StoreReq.sched[iD] - 1;
            StoreAllocSchedPlusOne[iSt][iD][ST_ALLOC_Q_SIZE - 1] = StoreReq.sched[iD] + 1;
