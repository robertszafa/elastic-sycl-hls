...
/// - mappings between instructions (loads and stores) in {F}, and sycl pipe
///   call instructions,
/// - call instructions to predicate pipes,
/// - the basic blocks of each bottleneck.
void collectLLVMValues(json::Object &report, Function &F, bool isMain,
  ...
  for (json::Value &bottleneck : *report["bottlenecks"].getAsArray()) {
    ...
    int idxBB = bottleneckOb["basic_block_idx"].getAsInteger().getValue();
...
struct CDDDTransform : PassInfoMixin<CDDDTransform> {
  ...
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &AM) {
    ...
    if (isMain) {
      // In the main kernel, we need to deal with in/out dependencies for all 
      // bottleneck kernels
      for (size_t iB = 0; iB < blocks.size(); ++iB) {
        // Read the result of the bottleneck kernels at the end of the block.
        changeVal2PipeRead(F, depOut[iB], blocks[iB], true);

        // Delete all instructions except the terminator and the above reads.
        SmallVector<Instruction *> exceptions;
        exceptions.push_back(blocks[iB]->getTerminator());
        llvm::append_range(exceptions, llvm::make_first_range(depOut[iB]));
        deleteInstructions(blocks[iB], exceptions);

        // Now insert pipe writes supplying in dependencies into the bottleneck.
      ...
      // One cluster of everything if this is a bottleneck kernel function.
