; ModuleID = '.matching.cpp_mem2reg.ll'
...
if.end:                                           ; preds = %if.then, %land.lhs.true, %while.body
  %out_scalar.1 = phi i32 [ %add13, %if.then ], [ %out_scalar.02, %land.lhs.true ], [ %out_scalar.02, %while.body ]
  %add14 = add nsw i32 %i.01, 1
  %cmp = icmp slt i32 %add14, 1000
  br i1 %cmp, label %while.body, label %while.end

while.end:                                        ; preds = %if.end
  %out_scalar.0.lcssa = phi i32 [ %out_scalar.1, %if.end ]
  ret i32 %out_scalar.0.lcssa
}
