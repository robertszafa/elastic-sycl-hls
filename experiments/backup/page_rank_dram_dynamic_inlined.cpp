...
std::vector<event> StreamingMemory(queue &q) {
  ...
  constexpr uint NUM_LOADS = DI.NUM_LOADS;
  constexpr uint NUM_STORES = DI.NUM_STORES;
  constexpr uint LOOP_DEPTH = DI.MAX_LOOP_DEPTH; 
...
double page_rank_kernel(queue &q, const std::vector<int> &h_row_ptr,
  ...
  q.single_task<class AGU_l0>([=]() [[intel::kernel_args_restrict]] {
    ...
    for (uint iter = 0; iter < maxIters; ++iter) {
      ...
      for (uint i = 0; i < numNodes; i++) {
        ...
        st_req.addr = uint(reinterpret_cast<uintptr_t>(p_new + i));
  ...
  q.single_task<class AGU_l1>([=]() [[intel::kernel_args_restrict]] {
    ...
    for (uint iter = 0; iter < maxIters; ++iter) {
      ...
      for (uint i = 0; i < numNodes; i++) {
        ...
        ld_req_p.addr = uint(reinterpret_cast<uintptr_t>(p+i));
        ...
        for (int j = 0; j < rowel; j++) {
          ...
          ld_req_p_new.addr = uint(reinterpret_cast<uintptr_t>(p_new + col_idx[local_curcol]));
          ld_req_p_new.posDepDist[1] = ld_req_p_new.addr > st_req_p_new.addr;
          LoadAddrPipes_p_new::PipeAt<0>::write(ld_req_p_new);

          st_req_p_new.addr = uint(reinterpret_cast<uintptr_t>(p_new + col_idx[local_curcol]));
  ...
  q.single_task<class AGU_l2>([=]() [[intel::kernel_args_restrict]] {
    ...
    for (uint iter = 0; iter < maxIters; ++iter) {
      ...
      for (uint i = 0; i < numNodes; i++) {
        ...
        ld_req_p_new.addr = uint(reinterpret_cast<uintptr_t>(p_new + i));
        LoadAddrPipes_p_new::PipeAt<1>::write(ld_req_p_new);

        st_req_p.addr = uint(reinterpret_cast<uintptr_t>(p + i));
  ...
  auto memEvents =
      StreamingMemory<4, LoadAddrPipes_p_new, LoadValPipes_p_new,
                      StoreAddrPipes_p_new, StoreValPipes_p_new, float>(q);

  auto memEvents2 =
      StreamingMemory<5, LoadAddrPipes_p, LoadValPipes_p, StoreAddrPipes_p,
                      StoreValPipes_p, float>(q);
