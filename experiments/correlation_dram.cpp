...
double correlation_kernel(queue &q, const int M, const int N,
  ...
  auto event = q.single_task<MainKernel>([=]() [[intel::kernel_args_restrict]] {
    ...
    const float sqrt_n = sqrtf(float(N));
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++) {
        auto res = data[i * N + j] - mean[j];
        res /= float_n * stddev[j];
        data[i * N + j] = res;
      }
    }
...
void correlation_cpu(const int M, const int N, std::vector<float> &data,
  ...
  for (int j = 0; j < M; j++) {
    ...
    float res = 0.0f;
    for (int i = 0; i < N; i++) {
      res += (data[i * N + j] - mean_j) * (data[i * N + j] - mean_j);
    }
    res /= float_n;
    res = sqrtf(res);
    res = res <= eps ? 1.0f : res;
