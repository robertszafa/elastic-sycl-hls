...
void loop_imperfect(dout_t dist[10*10]);
# 94 "loop_imperfect.c" 2

void loop_imperfect(dout_t dist[10*10]) {_ssdm_SpecArrayDimSize(dist, 100);
    for (int k=0; k < 10; ++k) {
      for (int i=0; i < 10; ++i) {
        for (int j=0; j < 10; ++j) {
          if (dist[i*10 + j] > dist[i*10 + k] + dist[k*10 + j])
            dist[i*10 + j] = dist[i*10 + k] + dist[k*10 + j];
        }
      }
    }
}
