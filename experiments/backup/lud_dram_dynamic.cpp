...
double lud_cmp_kernel(queue &q, std::vector<float> &h_A, const int kN) {
  ...
  q.single_task<class AGU_ij>([=]() [[intel::kernel_args_restrict]] {
    ...
    ld_req_t<NUM_STORES, LOOP_DEPTH> ld_req_2 {0u};
    ...
    ld_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + kN-1-1);
    ld_req_2.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1-1) * kN + kN-1-1);
    st_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + kN-1-1);
    for (int i = 0; i < kN; i++) {
      ld_req_1.sched[0]++;
      ld_req_2.sched[0]++;
      st_req_1.sched[0]++;

      ld_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + i-1);
      ld_req_2.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + (i-1) * kN + i-1);
      st_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + i-1);
      for (int j = 0; j < i; j++) {
        ld_req_1.sched[1]++;
        ld_req_2.sched[1]++;
        st_req_1.sched[1]++;

        ld_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + j);
        ld_req_1.posDepDist[0] = ld_req_1.addr > st_req_1.addr;
        LoadAddrPipes::PipeAt<0>::write(ld_req_1);

        ld_req_2.addr = reinterpret_cast<uintptr_t>(A + j * kN + j);
        ld_req_2.posDepDist[0] = ld_req_2.addr > st_req_1.addr;
        LoadAddrPipes::PipeAt<3>::write(ld_req_2);

        st_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + j);
  ...
  q.single_task<class AGU_ijk>([=]() [[intel::kernel_args_restrict]] {
    ...
    ld_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + kN-1-1-1);
    ld_req_2.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1-1-1) * kN + kN-1-1);
    for (int i = 0; i < kN; i++) {
      ld_req_1.sched[0]++;
      ld_req_2.sched[0]++;

      ld_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + i-1-1);
      ld_req_2.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + (i-1-1) * kN + i-1);
      for (int j = 0; j < i; j++) {
        ld_req_1.sched[1]++;
        ld_req_2.sched[1]++;

        ld_req_1.maxAddrAtLoop[2] = reinterpret_cast<uintptr_t>(A + i * kN + j-1);
        ld_req_2.maxAddrAtLoop[2] = reinterpret_cast<uintptr_t>(A + (j-1) * kN + j);
        for (int k = 0; k < j; k++) {
          ld_req_1.sched[2]++;
          ld_req_2.sched[2]++;

          ld_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + k);
          LoadAddrPipes::PipeAt<1>::write(ld_req_1);

          ld_req_2.addr = reinterpret_cast<uintptr_t>(A + k * kN + j);
  ...
  q.single_task<class AGU_il>([=]() [[intel::kernel_args_restrict]] {
    ...
    ld_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + (kN-1));
    st_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + (kN-1));
    for (int i = 0; i < kN; i++) {
      ld_req_1.sched[0]++;
      st_req_1.sched[0]++;

      ld_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + (kN-1));
      st_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + (kN-1));
      for (int l = i; l < kN; l++) {
        ld_req_1.sched[1]++;
        st_req_1.sched[1]++;

        ld_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + l);
        ld_req_1.posDepDist[1] = ld_req_1.addr > st_req_1.addr;
        LoadAddrPipes::PipeAt<4>::write(ld_req_1);

        st_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + l);
  ...
  q.single_task<class AGU_ilm>([=]() [[intel::kernel_args_restrict]] {
    ...
    ld_req_1.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1) * kN + (kN-1-1));
    ld_req_2.maxAddrAtLoop[0] = reinterpret_cast<uintptr_t>(A + (kN-1-1) * kN + (kN-1));
    for (int i = 0; i < kN; i++) {
      ld_req_1.sched[0]++;
      ld_req_2.sched[0]++;

      ld_req_1.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + i * kN + (i-1));
      ld_req_2.maxAddrAtLoop[1] = reinterpret_cast<uintptr_t>(A + (i-1) * kN + (kN-1));
      for (int l = i; l < kN; l++) {
        ld_req_1.sched[1]++;
        ld_req_2.sched[1]++;

        ld_req_1.maxAddrAtLoop[2] = reinterpret_cast<uintptr_t>(A + i * kN + (i-1));
        ld_req_2.maxAddrAtLoop[2] = reinterpret_cast<uintptr_t>(A + (i-1) * kN + l);
        for (int m = 0; m < i; m++) {
          ld_req_1.sched[2]++;
          ld_req_2.sched[2]++;

          ld_req_1.addr = reinterpret_cast<uintptr_t>(A + i * kN + m);
          LoadAddrPipes::PipeAt<5>::write(ld_req_1);

          ld_req_2.addr = reinterpret_cast<uintptr_t>(A + m * kN + l);
  ...
  auto memEvents = StreamingMemory<6, LoadAddrPipes, LoadValPipes,
                                   StoreAddrPipes, StoreValPipes>(q);
